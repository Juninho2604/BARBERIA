...
- **2026-05-28** — Fix Prisma + Alpine: `node:20-alpine` no trae OpenSSL y Prisma fallaba al cargar su engine binario. Añadido `apk add --no-cache openssl` al Dockerfile (base stage) y `binaryTargets = ["native", "linux-musl-openssl-3.0.x"]` al `schema.prisma` para que `prisma generate` descargue el binario correcto para Alpine 3.19+. — _este commit_
