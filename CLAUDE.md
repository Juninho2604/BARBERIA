...
- **Hito actual:** **M4 en curso** — disponibilidad (`GET /barbers/:id/availability`) y reservas (`POST/GET /appointments`, `PATCH /:id/cancel`) implementadas. Algoritmo de slots descuenta working hours del weekday, time off solapante y appointments activas. Reserva permite guest (sin sesión) o cliente autenticado. Concurrencia: re-chequeo dentro de transacción (devuelve 409 `SLOT_TAKEN`). Pendiente: endpoints de admin para gestionar `TimeOff` por barbero (M4.2).
...
- **Siguiente acción:** **M4.2** — endpoints admin para `TimeOff` (vacaciones/permisos del barbero) y luego M5 (flujo completo de reserva en el frontend).
...
| M4 — Disponibilidad + Reservas | En curso (M4.1 ✅ slots + appointments, M4.2 pendiente TimeOff admin) |
