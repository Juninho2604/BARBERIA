...
- **Rama activa:** `claude/quirky-ride-LctCn`
...
| M9 — Operación interna v1 | ✅ Cerrado (M9.1 roles granular + staff, M9.2 calendario operativo, M9.3 clientes/CRM lite; **backend real ✅** — guards por acción + endpoints staff/clients + PATCH appointment) |
...
- **2026-05-29** — **M9 / backend real (partes 3/4 y 4/4).** Cierra el "Backend real pendiente" que dejaron las partes 1/4 (frontend) y 2/4 (migración Prisma roles + campos User). Trabajo en rama `claude/quirky-ride-LctCn`.
  - **Parte 3/4 — guards por acción.** Nuevo `apps/api/src/auth/permissions.ts`: réplica autoritativa de `apps/web/src/lib/permissions.ts` (misma matriz `Action → Role[]` + helper `can`). Añadido `requireAction(action)` a `makeAuthGuards` (junto a `requireRole`). Migradas todas las rutas que usaban `requireRole("ADMIN")` a guards por acción: `services.ts` → `services.manage`, `barbers.ts` (+ working-hours) → `barbers.manage`, `timeoff.ts` → `barbers.manage`. `appointments.ts` cancel ahora permite cliente-dueño / barbero-asignado / `can(role,"appointments.cancel")` (antes era literal `role !== "ADMIN"`, que excluía MANAGER/RECEPTIONIST). **Regla:** la matriz es la única fuente; añadir un rol a una capacidad es un cambio en un solo sitio. Mantener en sync con la copia del frontend.
  - **Parte 4/4 — endpoints staff/clients + PATCH appointment.** `PATCH /appointments/:id` (status/notes; barbero asignado o `appointments.changeStatus`). Nuevo `routes/staff.ts`: `GET /staff` (usuarios con rol != CLIENT, proyectados a `StaffMember` con `barberId = barberProfile?.id`), `POST /staff/invite` (crea User sin password reclamable; si rol BARBER crea o vincula perfil Barber — vincular solo si el Barber tiene placeholder sin login ni citas, si no 409 `BARBER_NOT_LINKABLE`), `PATCH /staff/:id` (rol/isActive/nombre/teléfono, con guard transaccional "al menos un OWNER activo", ADMIN cuenta como OWNER). Nuevo `routes/clients.ts`: `GET /clients` (usuarios rol CLIENT con métricas agregadas total/completed/LTV/first/last calculadas sobre sus appointments), `GET /clients/:id` (+ historial cronológico desc con serviceName/barberName/priceCents), `PATCH /clients/:id/notes` (escribe `User.notes`). Rutas registradas en `server.ts` con prefijos `/staff` y `/clients`. Coinciden 1:1 con las rutas que `apps/web/src/lib/api.ts` ya consume.
  - **Login endurecido:** `POST /auth/login` ahora rechaza con 403 `ACCOUNT_DISABLED` si `isActive=false` y graba `lastLoginAt` en cada login exitoso (alimenta la columna que muestra `/admin/staff`).
  - **Verificado:** `prisma validate` ✅, `tsc --noEmit` API + web ✅, `pnpm --filter @barberia/api build` ✅. **Nota de seguridad:** el JWT lleva el rol del momento de emisión; un cambio de rol o desactivación no invalida tokens ya emitidos hasta que expiren (aceptable v1; revisar en M8 si se quiere revocación). El cálculo de clientes itera appointments en memoria — suficiente a escala barbería; si crece, mover a agregación SQL.
